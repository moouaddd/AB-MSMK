...
export default function ProductPage({ product }) {
  ...
  function onAddToCart(event){
    ...
    toast.success(`${count} ${product.name} añadido`, {id})
}
  return(
     <div className="container lg:max-w-screen-lg mx-auto py-12 px-6">
      ...
      space-y-8 md:space-y-0 md:space-x-12">
        ...
        <div className="w-full flex-1 max-w-md border border-opacity-0 rounded-md shadow-lg p-6 bg-white">
          <h2 className="text-3xl font-semibold">{product.name}</h2>
          <p className="pt2 flex items-center space-x-2">
            ...
            <div className="mt-4 border-t pt-4">
              ...
              <div className="mt-1 flex items-center space-x-3">
                <button disabled={count <= 1} onClick={() => setCount(count - 1)} className="p-1 rounded-md hover:bg-rose-100 hover:text-rose-500">
                  <MinusSmallIcon className="w-6 h-6 flex-shrink-0"></MinusSmallIcon>
                </button>
            ...
            <button onClick={onAddToCart} className="w-full mt-4 border border-lime-500 py-2 px-6 bg-lime-500 hover:bg-lime-600 hover:border-lime-600 focus:ring-4 focus:ring-opacity-50 focus:ring-lime-500 text-white disabled:opacity-50 disabled:cursor-not-allowed rounded">
               Añadir al carrito
            </button>
        </div>
      </div>
...
export async function getStaticPaths() {
  ...
  return {
    paths,
    fallback: "blocking", // Puedes ajustar esto según tus necesidades
  };
}
